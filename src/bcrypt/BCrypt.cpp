...
std::string BCrypt::gensalt(int rounds, const char *prefix) {
    HKU_CHECK(rounds >= 4 && rounds <= 31, "Invalid rounds");
    HKU_CHECK(strcmp(prefix, "2a") == 0 || strcmp(prefix, "2b") == 0,
...
std::string BCrypt::hashpw(const std::string &password, const std::string &salt) {
    ...
    char hashed[128] = {0};
    HKU_CHECK(bcrypt_hashpass(pwd.c_str(), salt.c_str(), hashed, 128) == 0, "Invalid salt");
