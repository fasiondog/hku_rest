    ...
    try {
        ...
        server->registerHttpHandle("GET", "/api/hello", 
            [](void* ctx) -> net::awaitable<void> {
                ...
                resp.set(http::field::server, "Hikuuu-HttpServer");
                resp.set(http::field::content_type, "application/json");
                resp.keep_alive(context->req.keep_alive());
                resp.body() = R"({"message": "Hello from HTTP Server!"})";
        ...
        std::cout << "  ✓ Connection pooling (max: " << HttpServer::get_max_connections() << ")" << std::endl;
        std::cout << std::endl;
