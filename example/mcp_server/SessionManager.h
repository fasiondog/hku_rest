...
struct SessionData {
    ...
    SessionData()
    : created_at(std::chrono::system_clock::now()), last_active(std::chrono::system_clock::now()) {}
    ...
    bool isExpired(int timeout_seconds = 3600) const {
        ...
        auto age = std::chrono::duration_cast<std::chrono::seconds>(now - last_active).count();
...
public:
    ...
    : m_timeout_seconds(timeout_seconds), m_max_sessions(max_sessions) {
        HKU_INFO("SessionManager initialized: timeout={}s, max_sessions={}", timeout_seconds,
                 max_sessions);
    }
     ...
     * @return 是否成功
     */
    bool setSessionMetadata(const std::string& session_id, const std::string& key,
     ...
     */
    nlohmann::json getSessionMetadata(const std::string& session_id, const std::string& key) {
        if (session_id.empty()) {
            return nullptr;
        }
    ...
    int cleanupExpiredSessions() {
        ...
        if (count > 0) {
            HKU_DEBUG("Cleaned up {} expired sessions (remaining: {})", count, m_sessions.size());
        }
