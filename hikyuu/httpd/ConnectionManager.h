...
public:
     ...
     */
    explicit ConnectionPermit(int permit_id) 
    : m_permit_id(permit_id), m_priority(0) {}
     ...
     */
    ConnectionPermit(int permit_id, int priority) 
    : m_permit_id(permit_id), m_priority(priority) {}
...
public:
    ...
    boost::asio::awaitable<ConnectionPermit> acquire() {
        ...
        while (expected < static_cast<int>(m_max_concurrent)) {
            if (m_current_count.compare_exchange_weak(
                ...
                HKU_DEBUG("Connection acquired: id={}, count={}/{}", 
        ...
        HKU_DEBUG("Connection waiting: current={}, waiting={}", 
                 m_current_count.load(), m_waiting_count.load());
        ...
        if (ec == boost::asio::error::operation_aborted) {
            ...
            while (true) {
                if (m_current_count.compare_exchange_weak(
                      ...
                      std::memory_order_acquire)) {
                    int permit_id = m_next_permit_id.fetch_add(1, std::memory_order_relaxed);
                    HKU_DEBUG("Connection acquired from queue: id={}, count={}/{}", 
                             permit_id, expected + 1, m_max_concurrent);
                    co_return ConnectionPermit(permit_id);
                }
            }
    ...
    void release(int permit_id) {
        ...
        HKU_DEBUG("Connection released: id={}, count={}/{}", 
                 permit_id, m_current_count.load(), m_max_concurrent);
