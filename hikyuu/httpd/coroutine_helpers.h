...
inline auto with_timeout(Duration timeout, Awaitable awaitable) {
    ...
    return
      [timeout, aw = std::move(awaitable)](
        auto& executor) -> boost::asio::awaitable<decltype(co_await std::declval<Awaitable>())> {
          boost::asio::steady_timer timer(executor);
...
inline auto parallel(Awaitables... awaitables) {
    return
      [... aw = std::move(awaitables)]() mutable
        -> boost::asio::awaitable<std::tuple<decltype(co_await std::declval<Awaitables>())...>> {
          // 并行启动所有任务
 ...
 * ```
 */
template <typename Func, typename BizResult = decltype(std::declval<Func>()())>
inline auto retry(Func func, int max_attempts, std::chrono::milliseconds delay_ms) {
    return [func, max_attempts, delay_ms]() mutable -> boost::asio::awaitable<BizResult> {
        std::exception_ptr last_exception;

        for (int attempt = 0; attempt < max_attempts; ++attempt) {
            try {
                if constexpr (std::is_same_v<BizResult, boost::asio::awaitable<
                                                          typename BizResult::value_type>>) {
...
inline auto try_op(Awaitable awaitable) {
    return
      [aw = std::move(
         awaitable)]() mutable -> boost::asio::awaitable<Expected<typename Awaitable::value_type>> {
          try {
