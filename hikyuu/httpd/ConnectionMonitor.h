...
public:
    ...
    void sample() {
        ...
        // 采集 Gauge 指标
        metrics.setGauge("hku_http_active_connections",
                         static_cast<double>(m_conn_mgr->getCurrentCount()));
        metrics.setGauge("hku_http_waiting_connections",
                         static_cast<double>(m_conn_mgr->getWaitingCount()));
        metrics.setGauge("hku_http_max_concurrent_connections",
                         static_cast<double>(m_conn_mgr->getMaxConcurrent()));
        metrics.setGauge("hku_http_acquire_queue_size",
                         static_cast<double>(m_conn_mgr->getWaitingCount()));

        // 采集 Counter 指标
        metrics.setCounter("hku_http_total_permits_issued",
                           static_cast<double>(m_conn_mgr->getTotalIssued()));
    }
