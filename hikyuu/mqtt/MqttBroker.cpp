    ...
    std::shared_ptr<std::optional<std::string>> const& username) 
{
    ...
    if (error != X509_V_OK) {
        ...
        HKU_LOG_ERROR("Certificate validation failed, depth: {}, message: {}", 
...
MqttBroker::Impl::Impl(const Parameter& param) {
    ...
    if (m_config.num_iocs == 0) {
        ...
        if (m_config.num_iocs == 0) m_config.num_iocs = 1;
    }
    ...
    // 创建 broker
    m_broker = std::make_unique<am::broker<epv_type>>(
        m_con_iocs[0]->get_executor()
    );
...
void MqttBroker::Impl::apply_socket_opts(auto& lowest_layer) {
    ...
    if (m_config.recv_buf_size != 0) {
        lowest_layer.set_option(
            net::socket_base::receive_buffer_size(m_config.recv_buf_size)
        );
    }
    if (m_config.send_buf_size != 0) {
        lowest_layer.set_option(
            net::socket_base::send_buffer_size(m_config.send_buf_size)
        );
    }
}
                ...
                while (!m_stop_requested.load()) {
                    ...
                    ssl_layer.set_verify_callback(
                        [this, username = std::make_shared<std::optional<std::string>>()](
                            bool preverified,
                            net::ssl::verify_context& ctx
                        ) {
                            return verify_certificate(m_config.verify_field, preverified, ctx, username);
                        }
                    );
                    ...
                    co_await ssl_layer.async_handshake(
...
MqttBroker::MqttBroker(const Parameter& param) 
    : m_impl(std::make_unique<Impl>(param)) {
}
