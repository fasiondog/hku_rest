...
constexpr BizErrCode BIZ_JSONRPC_INVALID_REQUEST =
  BIZ_MCP_ERR(-2600);  // JSON 合法，但不是有效请求（缺少 id/jsonrpc/method）
constexpr BizErrCode BIZ_JSONRPC_METHOD_NOT_FOUND = BIZ_MCP_ERR(-2601);  // 调用的方法 / 工具不存在
constexpr BizErrCode BIZ_JSONRPC_INVALID_PARAMS =
  BIZ_MCP_ERR(-2602);  // 参数缺失、类型错误、格式非法
constexpr BizErrCode BIZ_JSONRPC_INTERNAL_ERROR =
...
constexpr BizErrCode BIZ_MCP_VERSION_MISMATCH =
  BIZ_MCP_ERR(-2008);  // 客户端 / 服务端协议版本不兼容 或未初始化
